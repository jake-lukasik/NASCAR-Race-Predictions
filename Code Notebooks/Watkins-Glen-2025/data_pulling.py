...
def pull_data_from_driver_averages(url, driver_in_first, driver_in_fourth):
    ...
    for i in range(len(tables)):
        try:
            ...
            if "Driver" in temp.columns:
                if temp.iloc[0]["Driver"] == driver_in_first and temp.iloc[3]["Driver"] == driver_in_fourth: ## change this line to the best driver as of recent in the table
                    track_stats = temp
                    break
...
def pull_road_course_stats(url="https://www.driveraverages.com/nascar/tracktype_avg.php?trktype_id=4", driver_in_first=""):
    # send the HTTP request
    headers = {'User-Agent': 'Mozilla/5.0'}
    response = requests.get(url, headers=headers)
    ...
    for i in range(len(tables)):
        try:
            ...
            if "Driver" in temp.columns:
                if temp.iloc[0]["Driver"] == driver_in_first: ## change this line to the best driver as of recent in the season points
                    rd_course_stats = temp
                    break
        except Exception as e:
            print(f"Skipping table {i}: {e}")

    rd_course_stats.columns = rd_course_stats.columns.str.replace("  ", " ", regex=False).str.strip()
    for col in rd_course_stats.columns:
        if "Unnamed" in col:
            rd_course_stats = rd_course_stats.drop(columns=[col])

    return rd_course_stats